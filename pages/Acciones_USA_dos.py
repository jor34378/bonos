...
def cargar_datos_usa():
    ...
    for r in rutas:
        if os.path.exists(r):
            ...
            # Columna con signo corregido: positivo = ganancia, negativo = pérdida
            # Compra viene + (egreso), Venta viene - (ingreso)
            # Al sumar por trade: si ganaste, la suma es negativa → *-1 la hace positiva
            df['Flujo'] = df['Inversion_USA'] * -1
            return df
    return None
...
# PnL por trade usando la columna 'Flujo' (ya con signo correcto)
resumen = (
    df_cerrados
    .groupby(['Ticker', 'ID_Trade'])
    .agg(
        Resultado_USD  = ('Flujo',          'sum'),   # positivo = ganancia ✅
