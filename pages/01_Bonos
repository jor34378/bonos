...
# Función auxiliar para carga de MEP y Maestro (separadas para mayor claridad)
def cargar_entorno():
...
def procesar_cartera(df_mov, df_mep, df_maestro, precios_manuales):
    ...
    df_solo_c_v = df_solo_c_v.sort_values(['Ticker', 'Operado']) # IMPORTANTE: Ordenar por ticker también
    ...
    for (tipo, ticker), group in df_limpio.groupby(['Tipo', 'Ticker']):
        ...
        for _, row in group.iterrows():
            # CORRECCIÓN TX28: Umbral de reseteo más amplio (0.1 nominales)
            # Esto evita que residuos por comisiones dupliquen la posición
            if abs(c_neta) < 0.1: 
                c_nom, tot_ars_c, tot_usd_c = 0.0, 0.0, 0.0
                v_nom, tot_ars_v, tot_usd_v = 0.0, 0.0, 0.0
                c_neta = 0.0 # Forzamos el cero real
        ...
        # Redondeo final para determinar estado
...
try:
    ...
    if df_mov is not None:
        ...
        # Solo mostrar tickers con posición abierta para no llenar el sidebar
