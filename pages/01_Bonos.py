...
def cargar_datos():
    ...
    for ruta in archivos_csv:
        ...
        if nombre_base.startswith('~$'): continue # Ignorar temporales de Excel
        
        # Extraer ticker y limpiar (con .strip().upper() manual para el string)
        ...
        # Limpieza de duplicados EXACTOS dentro del archivo
    ...
    # --- CORRECCIÓN DEL ERROR 'Series' object has no attribute 'upper' ---
    # Usamos .str. para que Pandas entienda que son textos
    ...
    # --- DROP DUPLICATES PARA TX28 (Global) ---
    # Si el registro es igual en Ticker, Fecha, Operación, Cantidad y Precio, lo borra.
    ...
    if not archivos_maestro:
        ...
        # Corrección del error upper() aquí también
...
def procesar_cartera(df_mov, df_mep, df_maestro, precios_manuales):
    ...
    resumen = []
    # Agrupamos por Tipo y Ticker para el cálculo acumulado
    for (tipo, ticker), group in df_limpio.groupby(['Tipo', 'Ticker']):
        ...
        for _, row in group.iterrows():
            if abs(c_neta) < 0.1: # Reseteo si el trade se cerró
            ...
            if row['Operación'] == "Compra":
                ...
                c_neta -= abs(cant); v_nom += abs(cant)
                tot_ars_v += row['Importe_ARS']; tot_usd_v += row['Importe_USD']

        estado = "Abierto" if abs(c_neta) > 0.9 else "Cerrado"
