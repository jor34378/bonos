...
def load_data():
    try:
        ...
        if 'fecha' in df.columns:
            ...
            # Si no se llama 'fecha', buscamos columnas similares
...
if df_trades is not None:
    try:
        # --- 2. PROCESAMIENTO (Named Aggregation para evitar KeyError) ---
        df_cerrados = df_trades[(df_trades['Estado_Trade'] == 'Cerrado') & (df_trades['fecha'].notnull())].copy()

        # Agrupamos creando columnas nuevas explícitas
        ...
        # Métrica solicitada: Tamaño vs cuenta de 10k
        ...
        # --- 3. MÉTRICAS PONDERADAS (ENCABEZADO) ---
        def ponderado(df):
        ...
        m4.metric("Profit Factor", f"{p_factor:.2f}x")

        # --- 4. CUADRANTE SIMÉTRICO DE GRÁFICOS (2x2) ---
        st.write("---")
        fig_size = (8, 5) # Tamaño uniforme para todos
        ...
        # --- 6. LISTADO DE TRADES DETALLADO ---
        st.write("---")
        st.subheader("📜 Detalle de Operaciones Cerradas")
        
        # Formateo de visualización (Sin afectar cálculos)
        ...
        st.write("Columnas detectadas en el archivo:", list(df_trades.columns))
