...
def cargar_datos():
    ...
    df_mov["Operado"] = pd.to_datetime(df_mov["Operado"], dayfirst=True, errors='coerce')
    ...
    df_mep["Operado"] = pd.to_datetime(df_mep["Operado"]).dt.normalize()
...
def procesar_cartera(df_mov, df_mep, df_maestro, precios_ref):
    ...
    df_limpio = pd.merge_asof(
        df_solo_c_v, df_mep[['Operado', 'Cotiz_mep']],
        on='Operado', direction='backward', suffixes=('', '_hist')
    )
    ...
    for (tipo, ticker), group in df_limpio.groupby(['Tipo', 'Ticker']):
        ...
        p_exit_ars = precios_ref.get(ticker, ppp_ars) if estado == "Abierto" else (tot_ars_v / v_nom * 100 if v_nom > 0 else 0)
        p_exit_usd = p_exit_ars / MEP_HOY if estado == "Abierto" else (tot_usd_v / v_nom * 100 if v_nom > 0 else 0)
